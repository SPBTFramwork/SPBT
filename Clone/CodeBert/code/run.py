...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512,pool=None):
        ...
        with open(file_path, 'r') as f:
            ...
            for line in lines:
                obj = json.loads(line)
                data.append((obj['code1'], obj['code2'], obj['label'], tokenizer, args))
        # data = data[:100]
...
def evaluate(args, model, tokenizer, prefix="", dataset_type='test', pool=None, eval_when_training=False):
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = load_and_cache_examples(args, tokenizer, evaluate=False, test=False, pool=pool)
        if args.local_rank == 0:
    ...
    if args.do_test:
        ...
        model.to(args.device)
        pp_res = evaluate(args, model, tokenizer, dataset_type='test', pool=pool)
        # pc
        pc_res = evaluate(args, model, tokenizer, dataset_type='eval', pool=pool)
        ...
        model.to(args.device)
        cp_res = evaluate(args, model, tokenizer, dataset_type='test', pool=pool)
