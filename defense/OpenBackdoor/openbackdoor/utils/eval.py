...
def evaluate_detection(preds, labels, metrics: Optional[List[str]]=["FRR", "FAR"]):
    an = defaultdict(float)
    for metric in metrics:
        score = detection_metrics(preds, labels, metric=metric)
        if metric in ['F1', 'recall', 'FPR', 'precision']:
            score *= 100
        an[metric] = round(score, 2)
        # logger.info("{}: {}".format(metric, score))
    # logger.info(f"{json.dumps(an, indent=4)}")
    return an
